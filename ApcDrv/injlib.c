...
#define MY_POOL_TAG 'sjin'

//////////////////////////////////////////////////////////////////////////
// ke.h
//////////////////////////////////////////////////////////////////////////

LONG64 ready_shellcode = 0;
LONG64 code_reference_count = 0;

PVOID inject_data_64 = 0;
LONG64 inject_data_64_len = 0;
ULONG inject_data_64_entryoffset = 0;
ULONG inject_data_64_dataoffset = 0;
  ...
  )
{
  ...
  if (FALSE == ready_shellcode || NULL == (PVOID)inject_data_64)
  {
      goto Exit;
  }
  ...
  RtlCopyMemory(SectionMemoryAddress, (PVOID)inject_data_64, inject_data_64_len);
  UCHAR* entrypoint = (UCHAR*)SectionMemoryAddress+ inject_data_64_entryoffset;
  UCHAR* entrypointdata = (UCHAR*)SectionMemoryAddress + inject_data_64_dataoffset;
  Status = InjpQueueApc(UserMode,
      (PKNORMAL_ROUTINE)entrypoint,
      entrypointdata,     // Translates to 1st param. of LdrLoadDll (SearchPath)
  ...
  )
{
  ...
  if (NULL != inject_data_64)
  {
      ExFreePool(inject_data_64);
      inject_data_64 = NULL;      
  }
}
...
InjCreateInjectionInfo(
  ...
  _In_ HANDLE ProcessId, PEPROCESS Process, _In_ PPS_CREATE_NOTIFY_INFO CreateInfo
  )
{    
    if (FALSE == ready_shellcode)
    {
        return STATUS_UNSUCCESSFUL;
    }

    BOOLEAN IsChild = CreateInfo->ParentProcessId == (HANDLE)root_psid ? TRUE : FALSE;
    PLIST_ENTRY NextEntry = InjInfoListHead.Flink;
    ...
    if (FALSE == IsChild)
    {
        while (NextEntry != &InjInfoListHead)
        {
            PINJ_INJECTION_INFO current = CONTAINING_RECORD(NextEntry,
                INJ_INJECTION_INFO,
                ListEntry);

            if (CreateInfo->ParentProcessId == current->ProcessId)
            {
                IsChild = TRUE;
                break;
            }
  ...
  CapturedInjectionInfo->isWow64 = PsGetProcessWow64Process(Process) ? TRUE : FALSE;
  CapturedInjectionInfo->ProcessId = ProcessId;
  CapturedInjectionInfo->ForceUserApc = TRUE;
  CapturedInjectionInfo->Method = InjMethod;

  if (NULL != wcsstr(CreateInfo->CommandLine->Buffer, L"windbg.exe"))
  {
      CapturedInjectionInfo->NeedInject = FALSE;
  }
  else
  {
      CapturedInjectionInfo->NeedInject = TRUE;
  }
  ...
  )
{
  ...
  if ((LONG64)ProcessId == root_psid)
  {
      InterlockedExchange64(&root_psid, 0);
  }
}
  ...
  )
{
  ...
  if (CreateInfo)
  {
      if (0 != root_psid)
      {
          if (NULL == PsGetProcessWow64Process(Process))
          {
              InjCreateInjectionInfo(NULL, ProcessId, Process, CreateInfo);
          }
      }
  }
  else
...
)
{
  ...
  if (!InjectionInfo || InjectionInfo->IsInjected  ||FALSE == InjectionInfo->NeedInject)
  {
    return;
  }
...
NTSTATUS DispatchDeviceControl(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    InterlockedCompareExchange64(&ready_shellcode, 0, 1);

    //Enter Leave

    if (IOCTL_REGISTER_ROOT == ioControlCode) {        
        DbgPrint("Hello, World!\n");
        Irp->IoStatus.Status = STATUS_SUCCESS;
        Irp->IoStatus.Information = 0;
        InterlockedExchange64(&root_psid, (LONG64)PsGetCurrentProcessId());
    }
    else if (IOCTL_SETSHELLCODE_32 == ioControlCode) {
        Irp->IoStatus.Status = STATUS_SUCCESS;
        Irp->IoStatus.Information = 0;
        
    }
    else if (IOCTL_SETSHELLCODE_64 == ioControlCode) {
        Irp->IoStatus.Status = STATUS_SUCCESS;
        Irp->IoStatus.Information = 0;
        PInject_Code_Data code_data = Irp->AssociatedIrp.SystemBuffer;
        ULONG inlen = irpsp->Parameters.DeviceIoControl.InputBufferLength;
        ASSERT(sizeof(Inject_Code_Data) == inlen);
        ASSERT(code_data->code_data_len <=1024*1024);
                

        PVOID data = ExAllocatePoolWithTag(PagedPool,code_data->code_data_len, MY_POOL_TAG);
        if (NULL != data)
        {
            memcpy(data, (PVOID)code_data->code_data, code_data->code_data_len);
            PVOID old = NULL;
            //Enter
            old = inject_data_64;
            inject_data_64 = data;
            inject_data_64_len = code_data->code_data_len;
            inject_data_64_entryoffset = code_data->entrypoint_offset;
            inject_data_64_dataoffset = code_data->data_offset;
            //Leave
            if (NULL != old)
            {
                ExFreePool(old);
    ...
    ready_shellcode = (NULL != inject_data_64)?1:0;
